/**
 * Cadence — Prayer Pipeline (Google Apps Script)
 * =================================================
 * Bound to the prayer-collection Google Sheet.
 *
...
// ---------------------------------------------------------------------------
var CONFIG = {
  COL_ID:          2,    // B  — prayer id
  COL_PRAYER_TEXT: 3,    // C  — raw input
  ...
  KEYWORDS_COL:        1,           // column holding the keyword list (A). A "Keyword" header row is auto-skipped.
  MAX_KEYWORDS:        5,
  MAX_CATEGORIES:      3,

  // Allowed categories (edit this list freely). Model may pick only from these.
  CATEGORIES: [
    'Klassiker', 'Gottvertrauen & Hingabe', 'Tageszeiten', 'Lob & Preis',
    'Führung & Begleitung', 'Sehnsucht & Gottsuche', 'Krisenzeiten',
    'Schöpfung & Frieden', 'Schuld & Vergebung', 'Im Alter',
    'Tod & Ewigkeit', 'Dankbarkeit'
  ],
...
function chunkPrayerWithGemini(rawText) {
  ...
  var payload = {
    ...
    contents: [{ role: 'user', parts: [{ text: 'PRAYER:\n' + rawText }] }],
    generationConfig: {
      temperature: 0.2,
...
// ---------------------------------------------------------------------------
var SYSTEM_PROMPT = [
  'You format a German prayer into a natural reading layout for both text-to-speech and on-screen',
  ...
  '',
  'EXAMPLE INPUT:',
  'Himmlischer Vater, göttliche Quelle aller Versorgung, ich komme mit dankbarem Herzen vor Dich und danke Dir für die Segnungen, die ich bereits empfangen habe. Ich bitte Dich, Deine Fülle auszugießen. Amen.',
  '',
  'EXAMPLE OUTPUT:',
  '{"paragraphs":[',
  '["Himmlischer Vater, göttliche Quelle aller Versorgung,","ich komme mit dankbarem Herzen vor Dich","und danke Dir für die Segnungen, die ich bereits empfangen habe."],',
  '["Ich bitte Dich, Deine Fülle auszugießen."],',
  '["Amen."]',
  ']}'
].join('\n');
...
function generateTitleDescription_(ctx, need) {
  ...
  var rules = [
    'You curate a German prayer collection. Produce concise metadata in GERMAN.',
  ...
  if (need.title) {
    rules.push(
      ...
      '  German title that captures its theme or heart — max ~6 words, no trailing punctuation.',
      ''
    );
  }
  if (need.desc) {
    rules.push(
      'DESCRIPTION rules:',
      '- READ the prayer and write what it is about, in 1–2 German sentences — never more.',
      ...
      '  • "Salomos Tempelweihegebet aus dem Ersten Buch der Könige … Ein königliches Flehen um Gottes Nähe und Erhörung."',
      '  • "Augustinus\' bewegtes Liebesbekenntnis aus den Konfessionen – ein Aufatmen über die Erkenntnis … Klassisches Gebet der Umkehr und Gottessehnsucht."',
      '  • "Das Franz von Assisi zugeschriebene Friedensgebet – ein Sendungsauftrag, Liebe, Verzeihung, Hoffnung und Licht … zu tragen."',
      ''
    );
  }
...
function classifyTonalityStone_(prayer, need) {
  ...
  var rules = [
    'You classify German prayers. Judge ONLY from the prayer text.',
    'Return ONLY a JSON object with exactly these keys: { ' + wantList.join(', ') + ' }.',
    'No markdown, no commentary.',
    ''
  ];
...
// ===========================================================================
// Categories: 1–3 from CONFIG.CATEGORIES, based on the DESCRIPTION (+ text).
...
function runCatKwForMissingRows() {
  ...
  for (var row = CONFIG.HEADER_ROWS + 1; row <= lastRow; row++) {
    var raw = String(sheet.getRange(row, CONFIG.COL_PRAYER_TEXT).getValue()).trim();
    if (!raw) continue;
    var need = rowCatKwNeed_(sheet, row, allowedKw);
...
function runCatKwForRows(rowNumbers) {
  ...
  rowNumbers.forEach(function (row) {
    ...
    try {
      var out = classifyCatKw_(raw, desc, allowedKw, need);
...
function rowCatKwNeed_(sheet, row, allowedKw) {
  ...
  var hasCat = cats.some(function (v) { return !!normalizeEnum_(v, CONFIG.CATEGORIES); });
...
function classifyCatKw_(prayer, description, allowedKw, need) {
  var wantList = [];
  if (need.cat) wantList.push('"categories"');
  if (need.kw) wantList.push('"keywords"');

  var rules = [
    'You tag German prayers for a curated collection.',
    'Return ONLY a JSON object with exactly these keys: { ' + wantList.join(', ') + ' }.',
    'No markdown, no commentary.',
    ''
  ];
  ...
  if (need.kw) {
    rules.push(
      ...
      'over broad, generic ones: if a precise keyword fits (e.g. "Wohlstand", "Krankheit",',
      '"Morgengebet"), prefer it to a vague catch-all (e.g. "Vertrauen", "Bitte") that could',
  ...
  var result = {};
  if (need.cat) {
    result.categories = validateList_(obj.categories, CONFIG.CATEGORIES, CONFIG.MAX_CATEGORIES);
...
/** Every data row with prayer text whose AE isn't a valid value yet (excl. IDs 183–189). */
...
function runStyleForRows(rowNumbers) {
  ...
  rowNumbers.forEach(function (row) {
    ...
    try {
      // Hard rule: a "Klassiker" prayer is Classic — no model call needed.
      var cats = rowValues_(sheet, row, CONFIG.COL_CAT_FIRST, CONFIG.COL_CAT_LAST);
      var isKlassiker = cats.some(function (c) { return normalizeEnum_(c, CONFIG.CATEGORIES) === 'Klassiker'; });
      var style = isKlassiker ? 'Classic' : classifyStyle_(raw);
...
function classifyStyle_(prayer) {
  var rules = [
    'You classify a German prayer by its overall STYLE. Judge ONLY from the prayer text.',
